    ...
    public function update(Request $request)
    {
        foreach ($request->settings as $key => $value) {
            ...
            && !strpos($key, '_link')) {
                ...
                Setting::where('key', $key)->update(['value' => $value, 'category' => 'social']);
            }
        }
        if ($request->hasFile('settings.website_logo')) {
            $website_logo_setting = Setting::where('key', 'website_logo')->first();
            $image = $website_logo_setting->addMedia($request['settings']['website_logo'])->toMediaCollection(
                'website_logo'
            );
        ...
        if ($request->hasFile('settings.website_wide_logo')) {
            ...
            $image = $website_wide_logo_setting->addMedia($request['settings']['website_wide_logo'])->toMediaCollection(
                'website_wide_logo'
            );
        ...
        if ($request->hasFile('settings.website_icon')) {
            ...
            $image = $website_icon_setting->addMedia($request['settings']['website_icon'])->toMediaCollection(
                'website_icon'
            );
        ...
        if ($request->hasFile('settings.website_cover')) {
            ...
            $image = $website_cover_setting->addMedia($request['settings']['website_cover'])->toMediaCollection(
                'website_cover'
            );
