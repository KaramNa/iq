...
{
    public int $items_per_page = 5000;
    ...
    public function viewer(Request $request, $name, $page)
    {
        ...
        foreach ($items as $item) {
            $url = '<url><loc>' . str_replace(
                    '&',
                    '-',
                    route($route, $item)
                ) . '</loc><priority>1.0</priority><lastmod>' . gmdate(
                    DateTime::W3C,
    ...
    public function custom_links()
    {
        ...
        foreach ($this->custom_links as $custom_link) {
            $url = '<url><loc>' . $custom_link . '</loc><priority>1.0</priority><lastmod>' . gmdate(
                    DateTime::W3C,
