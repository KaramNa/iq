...
class UploadFilesHelper
{
    ...
    public static function generate_unique_filename($file, $options)
    {
        ...
        $filename = pathinfo($original_file_name, PATHINFO_FILENAME) . '-' . substr(
                str_shuffle("0123456789abcdefghijklmnopqrstvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"),
                0,
                6
            ) . '.' . $extension;
        return $filename;
    ...
    public static function store_file($options)
    {
        ...
        $validation = Validator::make(
            $options,
            ['source' => "required|mimes:" . self::get_validations($options["validation"]) . "|max:250000"]
        );
        if ($validation->fails()) {
        ...
        $temp_file = \App\Models\TempFile::create(
            ['name' => $options['temp_file_selector'], 'user_id' => auth()->check() ? auth()->id() : null]
        );
    ...
    public static function get_private_file(Request $request, HubFile $file)
    {
        ...
        if ($file->visibility == "PRIVATE") {
            if (\Auth::check() && (\Auth::user()->hasRole("ADMIN") || $file->user_id == \Auth::user()->id)) {
                return redirect(
                    Storage::disk($file->bucket_name)->temporaryUrl(
                        substr($file->path, 1) . $file->name,
                        now()->addHour()
                    )
                );
            } else {
                if ($file->type == "ticket_message") {
                    if (\Auth::check()) {
