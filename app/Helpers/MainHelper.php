...
class MainHelper
{
    ...
    {
        $ip = \UserSystemInfoHelper::get_ip();
        $total_req_per_minute = \App\Models\RateLimitDetail::where(
            'created_at',
            '>=',
            \Carbon::parse(now())->subMinutes(1)->format('Y-m-d H:i:s')
        )->orderBy('id', 'DESC')->count();
        if ($total_req_per_minute >= 2000) {
            $attacks = \App\Models\UnderAttack::where('status', 'UNDER_ATTACK')->where(
                'release_at',
                '>',
                \Carbon::parse(now())->format('Y-m-d H:i:s')
            )->count();
            if ($attacks == 0) {
                \App\Models\UnderAttack::create(
                    [
                        'status' => "UNDER_ATTACK",
                        'release_at' => \Carbon::parse(now())->addMinutes(30)->format('Y-m-d H:i:s')
                    ]
                );
                (new \App\Helpers\SecurityHelper)->enable_under_attack_mode();
            }
        }
        $limit_for_ip = \App\Models\RateLimitDetail::where('ip', \UserSystemInfoHelper::get_ip())->where(
            'created_at',
            '>=',
            \Carbon::parse(now())->subMinutes(1)->format('Y-m-d H:i:s')
        )->orderBy('id', 'DESC')->count();
        ...
        $last_insert = \App\Models\RateLimit::where('ip', $ip)->where(
            'created_at',
            '<=',
            \Carbon::parse(now())->addMinutes(3)
        )->first();
    ...
    public static function focus_urls($string)
    {
        ...
        return preg_replace(
            $url_regex,
            " <a href='$0' target='_blank' rel='nofollow' style='font-family: inherit;'>$0</a> ",
            urldecode(htmlspecialchars($string))
        );
    }
    ...
    public static function menuLinkGenerator(MenuLink $link)
    {
        if ($link->type == "CUSTOM_LINK") {
            ...
            return env("APP_URL") . substr(route('page.show', $page), strlen(env("APP_URL")) + 3);
//            return route('page.show', $page);
        } elseif ($link->type == "CATEGORY") {
            $category = Category::where('id', $link->type_id)->first();
            if ($category == null) {
                return env("APP_URL");
            }
            return env("APP_URL") . substr(route('category.show', $category), strlen(env("APP_URL")) + 3);
//            return  route('category.show', $category);
    ...
    {
        $temp_files = TempFile::where('name', $id)->with(['media'])->get();
